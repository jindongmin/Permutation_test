...
def _test(X, y, G, k, p, M):
    ...
    top_microbe = set(top["Unnamed: 0"])
    bot_microbe = set(bot["Unnamed: 0"])
...
def permutation_test(X, y, G, k, p):
    T = _test(X, y, G, k, p)
    for i in range(p):
